...
DocFlow 是一款**本地离线**文档格式转换工具，无需联网，文件不上传云端，安全可靠。支持 Windows、macOS 和 Linux。

---

## 系统要求

| 平台 | 最低版本 | Word/PPT → PDF 依赖 |
|------|---------|---------------------|
| Windows | Windows 10 / 11（64 位） | 本机已安装 Microsoft Office 或 WPS |
| macOS | macOS 12 Monterey 及以上 | LibreOffice（软件内可自动安装） |
| Linux | Ubuntu 20.04+ / Fedora 38+ 等主流发行版 | LibreOffice（软件内可自动安装） |
...
| 转换方向 | 输入格式 | 输出格式 | 依赖要求 |
|----------|----------|----------|----------|
| Word → PDF | .doc .docx | .pdf | Windows 需要 Office 或 WPS；macOS/Linux 需要 LibreOffice（可自动安装） |
| PPT → PDF | .ppt .pptx | .pdf | Windows 需要 Office 或 WPS；macOS/Linux 需要 LibreOffice（可自动安装） |
| PDF → Word | .pdf | .docx | 无 |
| PDF → PPT | .pdf | .pptx | 无 |
...
**Windows**：软件会在后台静默调用本机已安装的 Office 或 WPS，转换过程中请不要手动操作 Office / WPS 窗口。

**macOS / Linux**：软件依赖 LibreOffice 完成转换。首次使用时，若检测到未安装 LibreOffice，将自动弹出安装引导窗口：

- 点击 **"自动安装"** — 软件将尝试通过 Homebrew（macOS）、snap/flatpak/apt/dnf（Linux）等方式静默安装，实时显示进度日志
- 点击 **"打开官网"** — 前往 LibreOffice 官网手动下载安装，完成后点击 **"重新检测"**
- 点击 **"跳过"** — 取消此次转换，Word/PPT → PDF 功能将不可用
...
**Q：Word / PPT 转 PDF 报错？**  
A：
- **Windows**：请确认本机已安装 Microsoft Office 或 WPS，且版本正常可打开文件。
- **macOS / Linux**：请确认已安装 LibreOffice。首次使用时软件会自动弹出安装向导，也可前往 [libreoffice.org](https://www.libreoffice.org) 手动下载安装。
